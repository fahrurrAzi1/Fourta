@extends('layouts.app')
                ...
                <div class="container mt-3">
                    <div class="row">
                        <div class="col-md-10">
                            ...
                            <form  id="soalForm" action="{{ route('soal-admin.store') }}" method="post">
                                @csrf
                                <div class="form-row">
                                        ...
                                        <select name="kelas_id" id="kelas_id" class="form-control">
                                            ...
                                            @else
                                                @foreach($kelass as $kelas)
                                                    <option value="{{ $kelas->id }}">{{ $kelas->id_kelas }} - {{ $kelas->nama_sekolah }} - {{ $kelas->guru->name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    </div>
                        ...
                        <div class="col-md-10 mt-2 mb-3 ml-12 justify-content-center">
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <form action="{{ route('soal-admin.filter') }}" method="GET">
                                        <div class="form-row">
                                            <div class="form-group col-md-4">
                                                <label for="kelas_id">Pilih Kelas</label>
                                                <select name="kelas_id" id="kelas_id" class="form-control">
                                                    @foreach($kelass as $kelas)
                                                        <option value="{{ $kelas->id }}" {{ request('kelas_id') == $kelas->id ? 'selected' : '' }}>
                                                            {{ $kelas->id_kelas }} - {{ $kelas->nama_sekolah }} - {{ $kelas->guru->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </div>
    ...
    <script>
        ...
        $('#saveChanges').click(function() {
            var form = $('#editForm');
            var url = '{{ route('soal-admin.update', ':id') }}';
