@extends('layouts.app')
    @section('header')
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Halaman Admin: Kelola Jawaban') }}
        </h2>
                                            ...
                                            @foreach($kelass as $kelas)
                                                @if($kelas->guru_id) 
                                                    <button type="button" class="btn show-siswa mt-2 ml-2 font-weight-bold" data-kelas="{{ $kelas->id}}" data-sekolah="{{ $kelas->nama_sekolah }}">
                                                        Kelas {{ $kelas->id_kelas }} - ({{ $kelas->nama_sekolah }}) - {{ $kelas->guru->name }}
                                                    </button>
                                                @endif
                                            @endforeach
                                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.show-modal', function() {
                ...
                $.ajax({
                    url: '{{ route('admin.kelola-jawaban-admin.post') }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        jenis: selectedJenis,
            ...
            $('.show-siswa').click(function() {
                ...
                $.ajax({
                    url: '{{ route("admin.get-siswa-by-kelas") }}',
                    method: 'GET',
                    data: {
                        id_kelas: idKelas
                    },
            ...
            $(document).on('click', '.update-score', function(event) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: '{{ route('admin.update-skor-jawaban-admin') }}',
                            method: 'POST',
                            data: {
                                _token: '{{ csrf_token() }}',
                                id_siswa: idSiswa,
